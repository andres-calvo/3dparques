...
export const useGameStore = create<StoreInterface>((set, get) => ({
  ...
  matarJugador: () => {
    ...
    if (banderaError) {
        Swal.close()
        Swal.fire('No sea imbecil y juegue bien','No puede matar a nadie , se le pasa turno por pendejo','error');
        get().pasarTurno()
    }
  },
  ...
  siguienteTurno: () => {
    ...
    set({ jugadorActual: nuevoJugador ,instanciaJugadorActual:instanciaJuego.jugadores[nuevoJugador]});
  },
  pasarTurno: () => {
    ...
    jugadoresPosibles.forEach(jugador => {
      jugador.fichas.forEach(ficha => {
        if(posicionesJugador.includes(ficha.posicion) && !posicionSegura.includes(ficha.posicion)){
  ...
  girarDados: () => {
    ...
    if (!instanciaJugador.puedeJugar) {
      ...
      if (isPar) {
        ...
        if (!ganoUnaFicha) {
            Swal.close()
            Swal.fire('Sacaste par','Si sacas tres pares de seguido, una ficha sera sacada del juego automaticamente. Vuelve a girar los dados','success');
        }
        const movimientosPosiblesFichas = instanciaJugador.fichas.map(ficha => mostrarMovimiento(ficha, [dado1.valor,dado2.valor,dado3.valor]))
